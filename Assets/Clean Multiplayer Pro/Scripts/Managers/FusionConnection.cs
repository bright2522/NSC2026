...
namespace AvocadoShark
{
    public class FusionConnection : MonoBehaviour, INetworkRunnerCallbacks
    {
        ...
        [Header("Scene")] [SerializeField] private string gameSceneName = "Rpvp";
        [SerializeField] private string lobbyReturnSceneName = "CreateRoom";
        public string LobbyReturnSceneName => lobbyReturnSceneName;
        ...
        Application.targetFrameRate = (int)Screen.currentResolution.refreshRateRatio.value;
            ...
            if (string.IsNullOrWhiteSpace(_playerName))
                _playerName = PlayerPrefs.GetString("MP_PLAYER_NAME", "Player");
        }
        ...
        {
            _roomEntryList.RemoveAll(e => e == null);
        ...
        private void InitialRoomListSetup()
        {
            ...
            // Destroy() above is deferred to end of frame, so a null-filter alone would
            // not prune this refresh's entries - everything except the entry currently
            // being edited (which unsubscribes from OnDestroyRoomEntries) was destroyed.
            _roomEntryList.RemoveAll(e => e == null || e != CurrentEntryBeingEdited);
        ...
        private void ContinueConnectToRunner()
        {
            ...
            else if (string.IsNullOrWhiteSpace(_playerName))
                _playerName = PlayerPrefs.GetString("MP_PLAYER_NAME", "Player");

            PlayerPrefs.SetString("MP_PLAYER_NAME", _playerName);
        ...
        // Fusion 2.0 signature. Kept alongside the 2.1 overload below so the same
        // script compiles no matter which Fusion 2.x the user has installed.
        ...
        public void OnSceneLoadDone(NetworkRunner runner)
        {
            ...
            if (!string.IsNullOrWhiteSpace(additiveSceneName) && Runner.IsSceneAuthority)
            {
                int buildIndex = GetSceneBuildIndex(additiveSceneName);
                if (buildIndex >= 0)
                {
                    var asyncOp = Runner.LoadScene(SceneRef.FromIndex(buildIndex), LoadSceneMode.Additive);
                    asyncOp.AddOnCompleted(_ => SpawnPlayerCharacter(Runner));
                    return;
                }
            }
