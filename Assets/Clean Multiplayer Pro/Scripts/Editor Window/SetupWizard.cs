...
namespace AvocadoShark
{
    ...
    public class SetupWizard : EditorWindow
    {
        ...
        void OnGUI()
        {
            ...
            // Photon changed their packaging with Fusion 2.1: the Asset Store version of
            // Photon Voice 2 (Realtime 4) corrupts a Fusion 2.1 (Realtime 5) project.
            // Detect and offer the automatic repair before anything else.
            if (PhotonConflictRepairer.DetectState() == PhotonConflictRepairer.PhotonState.Conflict)
            {
                EditorGUILayout.HelpBox(
                    "Photon conflict detected! Your project mixes Photon Realtime 4 (Asset Store " +
                    "Photon Voice 2) with Photon Realtime 5 (Fusion 2.1+). This breaks compilation " +
                    "and prevents this wizard from applying changes.",
                    MessageType.Error);
                if (GUILayout.Button("Repair Photon Installation Now"))
                {
            ...
            bool isFusion21 = PhotonConflictRepairer.IsFusion21OrNewer;
            ...
            if (request.IsCompleted)
            {
                if (request.Status == StatusCode.Success)
                {
                    ...
                    GUI.enabled = !IsPhotonVoiceInstalled(); // Disable the GUI if Voice is installed
                    if (isFusion21)
                    {
                        // Fusion 2.1+ needs the 'Unity Voice SDK Realtime5' package from Photon's
                        // download page. The Asset Store version is Realtime 4 and incompatible.
                        if (GUILayout.Button("Download Photon Voice for Fusion 2.1 (Realtime5)"))
                        {
                            Application.OpenURL(PhotonConflictRepairer.VoiceRt5DownloadPage);
                            showVoiceHelpBox = true;
                        }

                        if (!IsPhotonVoiceInstalled() &&
                            GUILayout.Button("Import downloaded Voice package..."))
                        {
                            PhotonConflictRepairer.ImportDownloadedVoicePackage();
                        }

                        if (showVoiceHelpBox)
                        {
                            EditorGUILayout.HelpBox(
                                "Fusion 2.1 requires the 'Unity Voice SDK Realtime5' package " +
                                "(photon-voice-sdk_realtime5_*.unitypackage) from Photon's download page " +
                                "(sign-in required). Do NOT use the Asset Store version of Photon Voice 2 - " +
                                "it ships Realtime 4 and will corrupt the project.\n" +
                                "After downloading, click 'Import downloaded Voice package...' above.\n" +
                                "Remember to input your 'Photon Voice' App ID in " +
                    ...
                    // Photon's Voice-for-Fusion integration requires PhotonVoice.Fusion in
                    // NetworkProjectConfig's Assemblies To Weave.
                    if (IsPhotonVoiceInstalled())
                    {
                        bool weaved = PhotonConflictRepairer.IsVoiceFusionWeaved();
                        GUI.enabled = !weaved;
                        if (GUILayout.Button("Add PhotonVoice.Fusion to Assemblies To Weave"))
                        {
                            PhotonConflictRepairer.EnsureVoiceFusionWeaved();
                        }

                        GUI.enabled = true;
                        if (weaved)
                        {
                            GUILayout.Label("Weaved ✔️", checkStyle);
                        }
                    }

                    GUI.enabled = IsPhotonVoiceInstalled() && HasPun;
                    if (IsPhotonVoiceInstalled() && HasPun && GUILayout.Button("Remove PUN from Photon Voice"))
                    {
                        RemovePun();
                    }
                    ...
                    AddPackageButton("com.unity.cinemachine@2.10.7", "Cinemachine");
        ...
        bool AreAllPackagesInstalled()
        {
            ...
            if (!IsPhotonVoiceInstalled())
                return false;
            // Note: the Fusion Physics addon is no longer required - CMP's prefabs use
            // Fusion's built-in NetworkTransform (the 2.1 addon does not support Shared Mode).
            if (HasPun)
                return false;
